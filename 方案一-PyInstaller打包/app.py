...
def process_reverse_task(task_id, file_path, api_key, ext, original_name):
    task = tasks[task_id]
    task["status"] = "processing"
...
def reverse_upload():
    ...
    thread = threading.Thread(target=process_reverse_task, args=(task_id, file_path, api_key, ext, original_name), daemon=True)
    thread.start()
